...
// src / components / Dashboard / Sidebar / LeftSidebar.jsx;
...
const LeftSidebar = ({ isOpen, onClose }) => {
  ...
  // Use the custom hook for fetching logged-in user profile
  const {
  ...
  const handleProfileUpdate = (updatedUser) => {
    dispatch(setUser(updatedUser)); // Update the Redux store with the updated user
  };
    ...
    <>
      ...
      >
        ...
        {loggedInUser && loggedInUserProfile && !loading && !error && (
          <div className="mt-auto border-t border-gray-100 p-4">
            <Link
              to={`/profile/${loggedInUser.username}`} // Navigation to the user's profile
              className="block hover:bg-gray-50 rounded-lg p-2 transition-colors duration-200"
            >
              <SidebarProfile
                name={loggedInUserProfile.username}
